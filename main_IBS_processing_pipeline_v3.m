...
% for each speaker-listener identity
...
%% Separate data by speaker/listener relationship
...
%     brainWhenSpeaks_A = [];
%     w = 1;
%     % Loop over each speaking turn for Speaker A
%     for j = 1:size(spkrA_sgmts,1)
%         
%         % Define the initial speaking turn interval
%         spkrStart = round(spkrA_sgmts(j,1));
%         spkrEnd   = round(spkrA_sgmts(j,2));
%         intervals = [spkrStart, spkrEnd];  % each row is an interval
%         
...
%             newIntervals = [];
%             
%             % Process each current interval 
%             for k = 1:size(intervals, 1)
%                 intStart = intervals(k, 1);
%                 intEnd   = intervals(k, 2);
%                 
%                 % No overlap: mask is completely before or after the interval.
%                 if maskEnd < intStart || maskStart > intEnd
%                     newIntervals = [newIntervals; intStart, intEnd];
%                 else
%                     % Check if mask fully covers the interval:
%                     if maskStart <= intStart && maskEnd >= intEnd
%                         % Entire interval is masked out; do not add anything.
%                         continue
%                     else
%                         % Case where mask is fully inside the interval: split into two parts.
%                         if maskStart > intStart && maskEnd < intEnd
%                             left_length = maskStart - intStart;
%                             right_length = intEnd - maskEnd;
%                             if left_length >= 1000
%                                 newIntervals = [newIntervals; intStart, maskStart - 1]; %#ok<*AGROW> 
%                             end
%                             if right_length >= 1000
%                                 newIntervals = [newIntervals; maskEnd + 1, intEnd];
%                             end
%                         % Partial overlap at beginning: adjust start of interval.
%                         elseif maskStart <= intStart && maskEnd < intEnd
%                             newIntStart = maskEnd + 1;
%                             newIntervals = [newIntervals; newIntStart, intEnd];
%                         % Partial overlap at end: adjust end of interval.
%                         elseif maskStart > intStart && maskEnd >= intEnd
...
%             end
%             
%             % Update intervals after processing this mask.
%             intervals = newIntervals;
%             
%             % If no intervals remain, exit the mask loop early.
%             if isempty(intervals)
%                 break;
%             end
%         end
%         
%         % Now, for each remaining interval, if it is at least 2000 samples, append it.
%         for seg = 1:size(intervals,1)
%             segStart = intervals(seg, 1);
%             segEnd   = intervals(seg, 2);
%             if (segEnd - segStart + 1) >= 2000
%                 newSegBegn = size(brainWhenSpeaks_A,2) + 1;
%                 newSegEnd = newSegBegn + (segEnd - segStart);
%                 tmp(w,:) = [segStart segEnd];
%                 w = w+1;
%                 brainWhenSpeaks_A(:, newSegBegn:newSegEnd) = brain_data(:, segStart:segEnd);
%             end
%         end
%         
%     end
...
brain_A_WhenSpeaks_A = cell(length(convIdx), 1);
brain_A_WhenSpeaks_B = cell(length(convIdx), 1);
brain_B_WhenSpeaks_A = cell(length(convIdx), 1);
brain_B_WhenSpeaks_B = cell(length(convIdx), 1);

counter = 1;
for i = convIdx'
    brain_data_A   = alpha_A{i};
    brain_data_B   = alpha_B{i};
    ...
    % Process brain A when A speaks
    brain_A_WhenSpeaks_A{counter} = hyper_split_speakers(brain_data_A, spkrA_sgmts, mask_data);
    
    % Process brain A when B speaks
    brain_A_WhenSpeaks_B{counter} = hyper_split_speakers(brain_data_A, spkrB_sgmts, mask_data);

    % Process brain B when A speaks
    brain_B_WhenSpeaks_A{counter} = hyper_split_speakers(brain_data_B, spkrA_sgmts, mask_data);
    
    % Process brain B when B speaks
    brain_B_WhenSpeaks_B{counter} = hyper_split_speakers(brain_data_B, spkrB_sgmts, mask_data);
...
% Initialize output cell arrays for Brain A and Brain B
out_brainA = {};
out_brainB = {};

% Loop through each conversation index (convIdx holds indices for HighInterest or LowInterest)
for i = 1:length(convIdx)
    convIdx_i = convIdx(i);
    % Extract the condition from sub_A (assumed to be in column 2)
    condition = sub_A{convIdx_i, 2};  
    index = sub_A{convIdx_i, 3};

    % For Brain A:
    % When Speaker A speaks
    out_brainA(end+1, :) = {brain_A_WhenSpeaks_A{i}, 'Speaker A', condition, index};
    
    % When Speaker B speaks
    out_brainA(end+1, :) = {brain_A_WhenSpeaks_B{i}, 'Speaker B', condition, index}; %#ok<*SAGROW> 
    
    % For Brain B:
    % When Speaker A speaks   
    out_brainB(end+1, :) = {brain_B_WhenSpeaks_A{i}, 'Speaker A', condition, index};
    
    % When Speaker B speaks
    out_brainB(end+1, :) = {brain_B_WhenSpeaks_B{i}, 'Speaker B', condition, index};

end

%% Add non-conversation trials
...
for i = 1:length(nonConvIdx)
    ...
    condition = sub_A{trialIdx, 2};  % should be "Listening" or "Silence"
    trialIndex = sub_A{trialIdx, 3};
   
    nonconv_brainA(end+1, :) = {alpha_A{trialIdx}, 'none', condition, trialIndex};
    nonconv_brainB(end+1, :) = {alpha_B{trialIdx}, 'none', condition, trialIndex};
end

% prepend non-conversation trials
final_out_brainA = [nonconv_brainA; out_brainA];
final_out_brainB = [nonconv_brainB; out_brainB];

% Add signal length to the right for later normalization
getSignalLength = @(data) size(data, 2);  

% Add the new column to Brain A
for i = 1:size(final_out_brainA, 1)
    final_out_brainA{i, 5} = getSignalLength(final_out_brainA{i, 1});
end

% Add the new column to Brain B
for i = 1:size(final_out_brainB, 1)
    final_out_brainB{i, 5} = getSignalLength(final_out_brainB{i, 1});
end
...
%% Compute Inter-Brain Coordinatin With time lags (Alpha FBand)

% Extract the relevant column
alpha_split_A = final_out_brainA(:,1);
alpha_split_B = final_out_brainB(:,1);

Nblock = numel(alpha_split_B);
Nch = size(alpha_split_A{1},1);
...
parfor bi = 1:Nblock                                   % Loop for each trial
    ...
    brain_A = alpha_split_A{bi};                          % get trial N from subject A
    brain_B = alpha_split_B{bi};                          % get trial N from subject B
...
% To-do: Calculate copula beforehand!
%% Time lagged results
...
% ROI
% labels_to_find = {'FCz', 'C4'};
% indices = find(ismember(channels, labels_to_find));

indices = central_idx;
high_interest_lagged = squeeze(mean(results(high_interest_idx,:,indices,indices),1));
...
% Plot just the mean
plot(time_lags, high_interest_chan_lagged, 'k', 'LineWidth', 2)

xlabel('lag (ms)', 'FontSize', 12);
ylabel('bits', 'FontSize', 12);
title("High Interest Mean", 'FontSize', 14, 'FontWeight', 'bold');


%% Explore results of averaged lags

% iff results contains time lags
around_zero = lags(95:107); % -8 to 8 ms
